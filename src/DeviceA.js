...
const width = 500;//specifying the width of the graph
...
const green= '#008000';
const dates='Date';
const matter='Particulate Matter1'
const deva='Device A';
const devb='Device B';
const devc='Device C';
class DeviceA extends Component{
    ...
    .tickFormat(d => `${d}`);
      ...
      componentWillReceiveProps(nextProps){//will get data from parent component.
          const{matter,matter1,matter2}=nextProps;
          if(!matter||!matter1 ||!matter2)return alert('Data not loaded');//if data is not loaded then just return with an alert.

          //For x-axis we take the min and max date in our json file.
          const timeDomain = d3.extent(matter,d=>d.t);
          //For y-axis we take the max value of PM1 from deviceA
          const tempMax=d3.max(matter,d=>d.p1)
          //Update the scales with the above two values as domain
          this.xScale.domain(timeDomain);
          this.yScale.domain([0,tempMax+50]);
          this.lineGenerator.x(d=>this.xScale(d.t));
          this.lineGenerator.y(d=>this.yScale(d.p1));
          //generates the line for PM1 value for the deviceA
          const highs=this.lineGenerator(matter);
          this.lineGenerator.y(d=>this.yScale(d.p1));
          const lows=this.lineGenerator(matter1);
          //generates the line for PM1 value for the deviceB
          this.lineGenerator.y(d=>this.yScale(d.p1));
          const mids=this.lineGenerator(matter2);
          //generates the line for PM1 value for the deviceC
...
//Then we create group elements.One for x-axis and one for y-axis. 
        return (
          <svg width={width} height={height} >
            ...
            <path d={this.state.mids} fill='none' stroke={green} strokeWidth='3' />
            <g>
